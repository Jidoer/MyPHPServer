...
func main() {
	...
	app.Get("/{path:path}", func(ctx iris.Context) {
		...
		if strings.Contains(path, ".") {
			if strings.Contains(path, "./") {
				...
				if strings.Contains(lastname, "/") {
					...
					if file.Exists(Root + path) {
						if lastname == ".php" {
							PHPcgi(ctx.ResponseWriter(), ctx.Request() /*cgibin*/, "/usr/lib/cgi-bin/php7.4", Root+path) //php
			...
			//No file
			// cc/bb/index/
			if (file.Exists(Root + path + "/index.php")) /* || file.Exists(Root + path +"/index.html") || file.Exists(Root + path + "/index")*/ {
				PHPcgi(ctx.ResponseWriter(), ctx.Request() /*cgibin*/, "/usr/lib/cgi-bin/php7.4", Root+path+"/index.php")
			} else {
				//403!
